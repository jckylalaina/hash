...
#saisir donnée
def saisit
saisit=gets.chomp.to_i
return saisit
end

#nombre du journaliste
def question1(journalist)
	journalist=journalist
	i=1
puts "Question:combien y-a t'il de journaliste dans cette arrays"
...
journalist=journalist
	x=journalist.select {|p| p=~ /[0123456789]/ }
puts x.count
end
#handle avec Aude
def question3(journalist)
	journalist=journalist
	a=journalist.select { |p| p=~ /[Aa]ude/}
	puts a.count
end
#majuscule
...
#une majuscule pas encore terminee
def question5(journalist)
	x=[]
	journalist=journalist
	puts journalist.select.count{|p| p=~ /[A-Z]/} 
...
journalist=journalist
puts journalist.select{|p| p=~ /_/}.count
	
	end


#triez par ordre alphabetique
def question7(journalist)
	journalist=journalist
 x=journalist.sort
puts x.count
end
#50 handle les plus court
def question8(journalist)
journalist=journalist
x=journalist.sort_by{|p| p.length}	
puts x[0..49]
end
#position de @epenser
...
def question9(journalist)
	...
	journalist.each do |p|
		if p == "@epenser"
         puts "@epenser est en #{i} position dans le tableaux"	
     else
